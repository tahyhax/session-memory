...
if (isset($_POST['form'])) {
    ...
    saveUserInfo($_POST['form'], $sessionId);

    sendResponse($_SESSION[$sessionId]);
...
if (!isset($_SESSION[$sessionId])) {
    $_SESSION[$sessionId]['counter'] = 0;
}
$_SESSION[$sessionId]['counter']++;
$_SESSION[$sessionId]['lastConnect'] = date('m/d/Y  H:i:s');
 ...
 */
function saveUserInfo($params, $sessionId)
{
    ...
    $_SESSION[$sessionId]['info'] = $data;

    $yearOfBirth = getYearOfBirth($data['age']);

    $_SESSION[$sessionId]['info']['yearOfBirth'] = $yearOfBirth;
    ...
    <div class="wrap__inner session">
        <div class="session__info session-info">
            <div class="session-info__text">
                <?php
                if (isInfo($_SESSION[$sessionId])) {
                    echo renderUseInfo($_SESSION[$sessionId]['info']);
                }
                ?>
                You have been here
                <span class="session-info__connection-count connection-count"
                ><?=$_SESSION[$sessionId]['counter']?></span> <?=ngettext("time", "times", $_SESSION[$sessionId]['counter'])?>. Your the last visit was
                <time
                    datetime="<?=$_SESSION[$sessionId]['lastConnect'] ?>"
                    class="session-info__connection-last connection-last"
                ><?=$_SESSION[$sessionId]['lastConnect'] ?>
                </time
                >
                .
            </div>
